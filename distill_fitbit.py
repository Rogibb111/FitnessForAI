...
def main():
    ...
    for idx, csv_path in enumerate(csv_paths, start=1):
        if show_progress:
            _print_progress(idx, total_csv, os.path.relpath(csv_path, start=input_root))
        ...
        for row in rows_iter:
            ...
            if session_mode:
                ...
                # Duration
                dur_field = first_value(row, headers, ["duration", "length", "elapsed time"]) or first_value(row, headers, ["minutes"]) 
                ...
                distance = num_value(row, headers, ["distance", "km", "kilometer", "kilometre", "miles", "mi", "meters", "metres"])
                steps_v = num_value(row, headers, ["steps", "step count", "stepcount", "step"])
                avg_hr = num_value(row, headers, ["average heart", "avg heart", "avg hr", "average hr", "avg bpm", "average bpm", "mean hr"])
                max_hr = num_value(row, headers, ["max heart", "max hr", "peak heart", "max bpm"])
                elev_gain = num_value(row, headers, ["elevation gain", "elevation (m)", "elevation gain (m)", "elevation gain (ft)", "elev gain", "ascent", "climb"])
                azm_total = num_value(row, headers, ["active zone minutes", "azm", "zone minutes"])
                azm_fat = num_value(row, headers, ["fat burn minutes", "azm - fat burn", "active zone minutes - fat burn", "fat burn zone minutes", "fat burn"])
                azm_cardio = num_value(row, headers, ["cardio minutes", "azm - cardio", "active zone minutes - cardio", "cardio zone minutes", "cardio"])
                azm_peak = num_value(row, headers, ["peak minutes", "azm - peak", "active zone minutes - peak", "peak zone minutes", "peak"])
                ...
                if start_dt or end_dt or duration_min is not None or activity_type:
                    ...
                    # Drop None values for cleanliness but keep internal enrichment fields
                    public = {k: v for k, v in rec.items() if k not in ("_start_dt", "_end_dt") and v is not None}
                    # Attach back internal fields (even if None is fine, but they can be skipped too)
                    public["_start_dt"] = start_dt
                    public["_end_dt"] = end_dt
                    sessions_buffer.append(public)

                # Aggregate to daily workout metrics
                if date_str and duration_min is not None:
                    aggregate_value(daily_agg, date_str, "workout_minutes", float(duration_min))
                    aggregate_value(daily_agg, date_str, "workout_count", 1.0)
            ...
            if date_str:
                for h, mk in zip(headers, header_metric_keys):
                    ...
                    if val is None:
                        continue
                    metric_hits[mk] += 1
                    aggregate_value(daily_agg, date_str, mk, val)
...
Usage:
    python distill_fitbit.py --input Fitbit --output distilled
